...
Expected: 180 passed (170 + 9 new ask tests + 1 regression for the tree fix), 0 failed.

> **Executed note (deviation):** the plan's `list_tree` on a missing workspace raised
> `FileNotFoundError` (sandbox workspace dirs are not created for archived jobs). Fixed
> in `src/clio/tree.py` (`list_tree` returns `[]` for a missing dir) with regression test
> `test_list_tree_missing_dir_is_empty` in `tests/test_tree.py`. All five files committed
> in `7979806`.
...
Run: `python -m pytest -q`
Expected: 183 passed (180 + 3), 0 failed.
...
Run: `python -m pytest -q`
Expected: 184 passed (183 + 1), 0 failed.
...
- [ ] **Step 2: Full suite** — `python -m pytest -q` → 184 passed
