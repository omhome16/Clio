...
async def test_max_steps_caps_and_marks_not_ok(tmp_path):
    ...
    agent = _agent(fake, tmp_path, max_steps=3)
    report = await agent.run("x")
...
async def test_events_emitted(tmp_path):
    ...
    fake = FakeLLM(responses=[json.dumps({"tool": "list_tree", "args": {}}), json.dumps({"final": "ok"})])
    agent = _agent(fake, tmp_path, bus=bus)
    await agent.run("x")
...
async def test_compaction_trims_context(tmp_path):
    fake = FakeLLM(handler=lambda messages, model: json.dumps({"final": "done"}))
    agent = _agent(fake, tmp_path, max_context_chars=200)
    big = "x" * 10_000
    report = await agent.run(big)
    assert report.ok
    compacted = [m for m in fake.calls[0] if "dropped to fit budget" in m.content]
    assert compacted
    assert sum(len(m.content) for m in fake.calls[0]) <= 200
